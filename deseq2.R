...
getwd()  # "/home/hugj2006/ML/htseq_count"
files<-list.files()
for(f in files)
{
    x <- read.table(f,sep="\t")
...
write.table(all.count, file="count.raw.txt", row.names=FALSE, sep="\t")


count<-read.table(file="count.raw.txt", header=TRUE, sep="\t")
...
coldata$sample<-gsub(".namesort.bam.txt","",rownames(coldata))
coldata$flowering <-"F"
